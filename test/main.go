...
func main() {
	...
	envErr := fmt.Errorf("env vars LINODE_INSTA***REMOVED***E_ID and LINODE_VOLUME_ID must be set")

	var linodeInstanceID int
	var linodeVolumeID int
	var err error

	if linodeInstanceID, err = strconv.Atoi(os.Getenv("LINODE_INSTA***REMOVED***E_ID")); err != nil {
		log.Fatal(envErr)
	}
